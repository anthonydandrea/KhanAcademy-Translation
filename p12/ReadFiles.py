...
def Activate_The_Beast(string,writer):
    import argparse
    ...
    results = fa.translate(string)
    ##print(args.string)


    writer.writerow({'Source String': string, 'Google Translate': results['Google'], 'Yandex Translate': results['Yandex']})
...
def run(f):
    ...
    for index in range(len(fileArray)):
        if '<trans-unit' in fileArray[index]:
            ...
            src = fileArray[index+1]
            ##print('source:',src[16:fileArray[index+1].index('</')])
            ##print('sourceIG:',src[src.index('<source>')+9:src.index('</source>')])
            ##print('translation:',stringArray[index+2][stringArray[index+2].index('>')+1:len(stringArray[index+2])])
            ##print(stringArray[index+1][8:stringArray[index+1].index('</')])
            ##print(stringArray[index+2][stringArray[index+2].index('>'):stringArray[index+2].index("</")])
            ##print('\n\n'
            try:
...
def Thread_On(fileName,strings):
    #print(fileName)
    csvfile = open(fileName,'w')
    #with open(fileName, 'w') as csvfile:
    print('opening',fileName)
    fieldnames = ['Source String', 'Google Translate', 'Yandex Translate']
...
if __name__ == "__main__":
    import csv
    import _thread
    f = open('../HighPriorityContent.xliff')
    strings = run(f)
    #print('num Strings:',len(strings))
    numThreads = 1
    Thread_On('trans.csv',strings[24000:26000])
    #numStrings = int(len(strings)/numThreads)
    #for num in range(numThreads):
    #    try:
    #        #print('NumStrings/Thread:',int(len(strings)/numThreads))
    #        _thread.start_new_thread( Thread_On, ('translations_'+str(num)+'.csv', strings[num*numStrings:num*numStrings+numStrings]) )
    #    except:
